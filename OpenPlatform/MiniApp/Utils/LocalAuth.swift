...
//  MiniAppX
//
//  Created by w3bili on 2024/7/16.
//
...
internal enum LocalAuthBiometricAuthentication {
    case touchId
    case faceId
}

internal struct LocalAuth {
    private static let customKeyIdPrefix = "$#_".data(using: .utf8)!
    
    internal enum DecryptionResult {
        internal enum Error {
            case cancelled
            case generic
        }
...
internal struct BotBiometricsState: Codable, Equatable {
    internal struct OpaqueToken: Codable, Equatable {
    ...
    public static func create() -> BotBiometricsState {
...
internal func hexString(_ data: Data) -> String {
