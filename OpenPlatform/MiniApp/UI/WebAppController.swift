...
extension WebAppController.Node {
    ...
    private func checkWhiteListHost(_ url: String) -> Bool {
        let hosts = ["docs.openweb3.io","openweb3.io"]
        if let uri = URL(string: url), let host = uri.host, hosts.contains(host) {
            return true
        }
        return false
    }
    ...
    private func to110Request(url: String) async -> URLRequest? {
        if await true == self.controller?.webAppParameters.isDApp, !checkWhiteListHost(url) {
            
            let result = await OpenWeb3Repository.shared.get110LaunchInfo(url: url, id: self.controller?.webAppParameters.dAppDto?.id)
    ...
    fileprivate func sendSayHelloEvent() {
        let paramsString = "{msg: 'Welcome to IOS OpenWeb3 MiniApp!'}"
    ...
    fileprivate func invokeCustomMethod(requestId: String, method: String, params: String) {
       ...
       let isDeal = MiniAppServiceImpl.instance.appDelegate.customMethodProvider(app, method, params) { [weak self] result in
        ...
        if !isDeal, let appId = self.controller?.webAppParameters.miniAppId {
            Task {
                let result = await OpenWeb3Repository.shared.invokeCustomMethods(params: CustomMethodParams(appId: appId, method: method, params: params))
                DispatchQueue.main.async { [weak self] in
                    guard let weakSelf = self else {
                        return
                    }
...
extension WebAppController: UIViewControllerTransitioningDelegate {
    ...
    func onClickMenu(type: OW3MenuType) {
        ...
        case .TERMS:
            MiniAppServiceImpl.instance.openUrl(viewController: self, url: "https://docs.openweb3.io/docs/terms-of-service-for-mini-apps", webLaunchParams: self.webAppParameters)
        case .PRIVACY:
    ...
    func getVisibleMenus() -> [MenuItem] {
        var menus: [MenuItem] = [
            ...
            MenuItem(type: .TERMS, title: context.resourceProvider.getString(key: "Common.Terms") ?? "", icon: "icon_menu_user_agreement"),
        ]
