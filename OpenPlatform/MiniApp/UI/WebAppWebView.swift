...
private var openWeb3EventProxySource: (String) -> String = { webappName in 
...
internal final class WebAppWebView: BaseWebView {
    ...
    init(accountId: String, webAppName: String) {
        ...
        var handleOpenWeb3ScriptMessageImpl: ((WKScriptMessage) -> Void)?
        let openWeb3EventProxySource = WKUserScript(source: openWeb3EventProxySource(webAppName), injectionTime: .atDocumentStart, forMainFrameOnly: false)
        contentController.addUserScript(openWeb3EventProxySource)
        contentController.add(WeakGameScriptMessageHandler { message in
            handleOpenWeb3ScriptMessageImpl?(message)
        }, name: "performOpenWeb3Action")
        ...
        handleOpenWeb3ScriptMessageImpl = { [weak self] message in
    ...
    public func sendEvent(name: String, data: String?) {
        if(isTelegramWebApp) {
            ...
            sendOpenWeb3Event(name: name, data: data)
        }
    }
    ...
    private func sendOpenWeb3Event(name: String, data: String?) {
