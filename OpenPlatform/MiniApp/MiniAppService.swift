...
open class MiniAppService : NSObject {
    ...
    open func getCloudStoreValue(key: String, appId: String) async ->  Result<String?,ApiError> {
        return .failure(.invalidResponse)
    }
    
    open func setCloudStoreValue(key: String, value: String, appId: String) async ->  Result<Void,ApiError> {
        return .failure(.invalidResponse)
    }
...
internal final class MiniAppServiceImpl : MiniAppService {
    ...
    private var _webAppName = "OpenWeb3"
    ...
    override func getCloudStoreValue(key: String, appId: String) async -> Result<String?, ApiError> {
        
        let data = try? JSONSerialization.data(withJSONObject: ["keys": [key]], options: [])
            let dataString = String(data: data!, encoding: .utf8) ?? ""
        
        let result = await OpenWeb3Repository.shared.invokeCustomMethods(params: CustomMethodParams(appId: appId, method: "getStorageValues", params:dataString))
    ...
    override func setCloudStoreValue(key: String, value: String, appId: String) async -> Result<Void, ApiError> {
    ...
    override public func batchGetMiniApps(appIds: [String]) async -> Result<MiniAppResponse,ApiError> {
        return await OpenWeb3Repository.shared.batchGetMiniApp(appIds: appIds)
    }
    
    override func getMiniAppInfoById(appId: String) async -> Result<MiniAppDto,ApiError> {
        return await OpenWeb3Repository.shared.getMiniAppInfo(id: appId)
    }
    
    override func getMiniAppInfoByNames(botIdOrName: String, appName: String) async -> Result<MiniAppDto,ApiError> {
        return await OpenWeb3Repository.shared.getMiniAppInfo(botIdOrName: botIdOrName, appName: appName)
    }
    
    override func getDAppInfoById(dappId: String) async -> Result<DAppDto,ApiError> {
        return await OpenWeb3Repository.shared.getDAppInfo(id: dappId)
    }
    ...
    override public func preload(config: WebAppLaunchParameters) {
        ...
        let process : (String) -> Void = { appId in
            Task {
                let result = await OpenWeb3Repository.shared.getLaunchInfo(params: LaunchParams(url: nil, appId: appId, languageCode: DefaultResourceProvider.shared.getLanguageCode(), startParams: config.startParams, themeParams: generateWebAppThemeParams(resourceProvider: self.resourceProvider), peer: config.peer, platform: "IOS"))
    ...
    private func _launch(config: WebAppLaunchParameters)  -> IMiniApp? {
        ...
        if let config = config as? WebAppLaunchWithDialogParameters {
            ...
            if let dappId = config.id {
                Task {
                    let result = await OpenWeb3Repository.shared.getDAppInfo(id: dappId)
                    DispatchQueue.main.async { [weak self] in
    ...
    private func launchWithUrl(parentViewController: UIViewController,
        ...
        if let resolveUrl = resolveUrlImpl(url: resoleveUrl, basePaths: _context!.mePaths) {
            ...
            case let .app(appId, querys, urlParams):
                ...
                Task {
                    let result = await OpenWeb3Repository.shared.getMiniAppInfo(id: appId)
                    DispatchQueue.main.async { [weak self] in
                        guard let weakSelf = self else {
                            return
                        }
            ...
            case let .peer(.name(botName), .appStart(miniAppName, statApp, urlParams)):
                ...
                Task { [weak self] in
                    let result = await OpenWeb3Repository.shared.getMiniAppInfo(botIdOrName: botName, appName: miniAppName)
            ...
            case let .dapp(appId, querys):
                Task {  [weak self] in
                    let result = await OpenWeb3Repository.shared.getDAppInfo(id: appId)
                    DispatchQueue.main.async {
                        guard let weakSelf = self else {
                            return
                        }
