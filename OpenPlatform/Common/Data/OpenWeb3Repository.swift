...
internal let API_HOST = "https://api.m.openweb3.io"
internal let API_HOST_DEV = "https://dev-mapi.mtsocialdao.com"

internal class OpenWeb3Repository : OpenWeb3Datasource {
    
    static let shared: OpenWeb3Datasource  = OpenWeb3Repository()
    ...
    private func getBaseHost() -> String {
        return OpenPlatformPluginImpl.getInstance().apiHost ?? (OpenPlatformPluginImpl.getInstance().isDev ? API_HOST_DEV : API_HOST)
    }
    ...
    func sendRequest<T: Decodable>(request: URLRequest, withToken: Bool, retryCount: Int = 1) async -> Result<T, ApiError> {
        ...
        configuration.timeoutIntervalForResource = 60  // Total resource timeout
                    ...
                    do {
                        ...
                        do {
                            let openWeb3Error = try JSONDecoder().decode(OpenWeb3Error.self, from: data)
                            continuation.resume(returning: .failure(.requestFailed(statusCode: openWeb3Error.code, message: openWeb3Error.error)))
                        } catch {
