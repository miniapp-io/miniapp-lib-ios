...
internal protocol PostboxCoding {
...
internal func postboxEncodableTypeHash(_ type: Any.Type) -> Int32 {
    let string = "\(type)"
    let hash = murMurHashString32(string)
    return hash
}

internal func declareEncodable(_ type: Any.Type, f: @escaping(PostboxDecoder) -> PostboxCoding) {
    let string = "\(type)"
    let hash = murMurHashString32(string)
...
internal func declareEncodable(typeHash: Int32, _ f: @escaping(PostboxDecoder) -> PostboxCoding) {
...
internal func persistentHash32(_ string: String) -> Int32 {
    return murMurHashString32(string)
}

private let emptyMemory = malloc(1)!

internal class MemoryBuffer: Comparable, Hashable, CustomStringConvertible {
...
internal final class WriteBuffer: MemoryBuffer {
    public var offset = 0
...
internal final class ReadBuffer: MemoryBuffer {
    public var offset = 0
...
internal enum ObjectDataValueType {
    case Int32
    case Int64
    case Bool
    case Double
...
internal final class PostboxEncoder {
...
internal final class PostboxDecoder {
