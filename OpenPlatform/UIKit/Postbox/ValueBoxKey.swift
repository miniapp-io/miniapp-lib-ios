...
internal struct ValueBoxKey: Equatable, Hashable, CustomStringConvertible, Comparable {
    ...
    internal init(_ buffer: MemoryBuffer) {
    ...
    internal func getMemoryBuffer(_ offset: Int, length: Int) -> MemoryBuffer {
        assert(offset >= 0 && offset + length <= self.length)
...
internal func mdb_cmp_memn(_ a_memory: UnsafeMutableRawPointer, _ a_length: Int, _ b_memory: UnsafeMutableRawPointer, _ b_length: Int) -> Int
{
    var diff: Int = 0
    var len_diff: Int = 0
    var len: Int = 0
