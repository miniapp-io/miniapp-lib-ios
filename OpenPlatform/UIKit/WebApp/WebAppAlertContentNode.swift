...
private final class WebAppAlertContentNode: AlertContentNode {
    ...
    init(theme: AlertControllerTheme, resourceProvider: IResourceProvider, peerName: String, requestWriteAccess: Bool, actions: [TextAlertAction]) {
...
//        }
    ...
    override func updateTheme(_ theme: AlertControllerTheme) {
        self.textNode.attributedText = NSAttributedString(string: resourceProvider.getString(key: "WebApp.AddToAttachmentAdd") ?? "", font: Font.bold(17.0), textColor: theme.primaryColor, paragraphAlignment: .center)
        ...
        self.allowWriteLabelNode.attributedText = formattedText(resourceProvider.getString(key: "WebApp.AddToAttachmentAllowMessages") ?? "", color: theme.primaryColor)
...
internal func addWebAppToAttachmentController(context: AccountContext, peerName: String, /*icons: [AttachMenuBots.Bot.IconName: TelegramMediaFile],*/ requestWriteAccess: Bool, completion: @escaping (Bool) -> Void) -> AlertController {
    let resourceProvider = context.resourceProvider
    
    var dismissImpl: ((Bool) -> Void)?
    var getContentNodeImpl: (() -> WebAppAlertContentNode?)?
    let actions: [TextAlertAction] = [TextAlertAction(type: .genericAction, title: resourceProvider.getString(key: "Common.Cancel") ?? "", action: {
        dismissImpl?(true)
    }), TextAlertAction(type: .defaultAction, title: resourceProvider.getString(key: "WebApp.AddToAttachmentAdd") ?? "", action: {
    ...
    let contentNode = WebAppAlertContentNode(theme: AlertControllerTheme(resourceProvider: resourceProvider, fontSize: .regular), resourceProvider: resourceProvider, peerName: peerName, /*icons: icons,*/ requestWriteAccess: requestWriteAccess, actions: actions)
