...
private final class WebAppLaunchConfirmationAlertContentNode: AlertContentNode {
   ...
   // private let avatarNode: AvatarNode
    ...
    init(context: AccountContext, theme: AlertControllerTheme, title: String, text: String, showMore: Bool, requestWriteAccess: Bool, actions: [TextAlertAction], morePressed: @escaping () -> Void) {
        //self.peer = peer
...
//        }
    ...
    override func updateTheme(_ theme: AlertControllerTheme) {
        ...
        self.moreButton.setAttributedTitle(NSAttributedString(string: self.resourceProvider.getString(key: "WebApp.LaunchMoreInfo") ?? "", font: Font.regular(13.0), textColor: theme.accentColor), for: .normal)
...
//        self.allowWriteLabelNode.attributedText = formattedText(strings.WebApp_AddToAttachmentAllowMessages(self.peer.compactDisplayTitle).string, color: theme.primaryColor)
    ...
    override func updateLayout(size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        let avatarSize = CGSize(width: 0.0, height: 0.0)
...
internal func webAppLaunchConfirmationController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)?, /*peer: EnginePeer, */requestWriteAccess: Bool = false, completion: @escaping (Bool) -> Void, showMore: (() -> Void)?) -> AlertController {
    
    var dismissImpl: ((Bool) -> Void)?
    var getContentNodeImpl: (() -> WebAppLaunchConfirmationAlertContentNode?)?
    let actions: [TextAlertAction] = [TextAlertAction(type: .defaultAction, title: context.resourceProvider.getString(key: "WebApp.LaunchOpenApp") ?? "", action: {
        ...
        dismissImpl?(true)
    }), TextAlertAction(type: .genericAction, title: context.resourceProvider.getString(key: "Common.Cancel") ?? "", action: {
        dismissImpl?(true)
    })]
    
    let title = ""//peer.compactDisplayTitle
    let text = context.resourceProvider.getString(key: "WebApp.LaunchConfirmation") ?? ""
    
    let contentNode = WebAppLaunchConfirmationAlertContentNode(context: context, theme: AlertControllerTheme(resourceProvider: context.resourceProvider, fontSize: .regular), title: title, text: text, showMore: showMore != nil, requestWriteAccess: requestWriteAccess, actions: actions, morePressed: {
        dismissImpl?(true)
        showMore?()
    })
    ...
    let controller = AlertController(theme: AlertControllerTheme(resourceProvider: context.resourceProvider, fontSize: .regular), contentNode: contentNode)
