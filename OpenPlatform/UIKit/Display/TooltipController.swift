...
internal protocol TooltipControllerCustomContentNode: ASDisplayNode {
    func animateIn()
    func updateLayout(size: CGSize) -> CGSize
}

internal enum TooltipControllerContent: Equatable {
    case text(String)
...
internal final class TooltipControllerPresentationArguments {
...
internal class TooltipController: ViewController, StandalonePresentableController {
    ...
    private var timeoutTimer: SignalTimer?
    ...
    private func beginTimeout() {
        if self.timeoutTimer == nil {
            let timeoutTimer = SignalTimer(timeout: self.timeout, repeat: false, completion: { [weak self] in
