...
extension CGFloat: Interpolatable {
    internal static func interpolator() -> (Interpolatable, Interpolatable, CGFloat) -> Interpolatable {
...
internal let listViewAnimationCurveSystem: (CGFloat) -> CGFloat = { t in
    return springAnimationSolver(t)
}

internal let listViewAnimationCurveLinear: (CGFloat) -> CGFloat = { t in
    return t
}

internal let listViewAnimationCurveEaseInOut: (CGFloat) -> CGFloat = { t in
    return bezierPoint(0.42, 0.0, 0.58, 1.0, t)
}

#if os(iOS)
internal func listViewAnimationCurveFromAnimationOptions(animationOptions: UIView.AnimationOptions) -> (CGFloat) -> CGFloat {
...
internal  final class ListViewAnimation {
...
internal func listViewAnimationDurationAndCurve(transition: ContainedViewLayoutTransition) -> (Double, ListViewAnimationCurve) {
    switch transition {
        case .immediate:
...
internal func scrollingRubberBandingOffset(offset: CGFloat, bandingStart: CGFloat, range: CGFloat, coefficient: CGFloat = 0.4) -> CGFloat {
