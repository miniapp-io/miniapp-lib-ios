...
internal final class ListViewBackingView: UIView {
...
internal enum ListViewVisibleContentOffset {
    case known(CGFloat)
    case unknown
    case none
}

internal enum ListViewScrollDirection {
    case up
    case down
}

internal struct ListViewKeepTopItemOverscrollBackground {
...
internal enum GeneralScrollDirection {
    case up
    case down
}
...
internal class ListView: ASDisplayNode, ASScrollViewDelegate, ASGestureRecognizerDelegate {
    internal struct ScrollingIndicatorState {
    ...
    private var itemReorderingTimer: SignalTimer?
    ...
    private func checkItemReordering(force: Bool = false) {
        ...
        if !force {
            self.itemReorderingTimer = SignalTimer(timeout: 0.025, repeat: false, completion: { [weak self] in
