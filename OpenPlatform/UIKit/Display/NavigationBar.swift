...
internal class SparseNode: ASDisplayNode {
...
internal class SparseContainerView: UIView {
...
internal final class NavigationBarTheme {
...
internal final class NavigationBarStrings {
...
internal final class NavigationBarPresentationData {
    public let resourceProvider: IResourceProvider
    public let strings: NavigationBarStrings
    public let theme: NavigationBarTheme?
    
    public init(resourceProvider: IResourceProvider, strings: NavigationBarStrings, theme: NavigationBarTheme? = nil ) {
...
internal enum NavigationPreviousAction: Equatable {
...
internal final class NavigationBackgroundNode: ASDisplayNode {
...
internal class BlurredBackgroundView: UIView {
    private var _color: UIColor?
...
internal protocol NavigationBarHeaderView: UIView {
}

internal class NavigationBar: ASDisplayNode {
    ...
    var resourceProvider: IResourceProvider
    ...
    private var title: String? {
        didSet {
            if let title = self.title {
                self.titleNode.attributedText = NSAttributedString(string: title, font: NavigationBar.titleFont, textColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_PRIMARY_TEXT_COLOR))
        ...
        get {
            ...
            if updateNodes {
                ...
                if let value = value {
                    switch value.role {
                        case .top:
                            if let transitionTitleNode = value.navigationBar?.makeTransitionTitleNode(foregroundColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_PRIMARY_TEXT_COLOR)) {
                        ...
                        case .bottom:
                            if let transitionBackButtonNode = value.navigationBar?.makeTransitionBackButtonNode(accentColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR)) {
                                self.transitionBackButtonNode = transitionBackButtonNode
                                self.buttonsContainerNode.addSubnode(transitionBackButtonNode)
                            }
                            if let transitionBackArrowNode = value.navigationBar?.makeTransitionBackArrowNode(accentColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR)) {
    ...
    public init(presentationData: NavigationBarPresentationData) {
        self.resourceProvider = presentationData.resourceProvider
        ...
        self.badgeNode = NavigationBarBadgeNode(fillColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR), strokeColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR), textColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BADGE_TEXT_COLOR))
        ...
        self.backButtonNode.color = self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR)
        self.backButtonNode.disabledColor = self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_DISABLED_BUTTON_COLOR)
        self.leftButtonNode.color = self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR)
        self.leftButtonNode.disabledColor = self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_DISABLED_BUTTON_COLOR)
        self.rightButtonNode.color = self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR)
        self.rightButtonNode.disabledColor = self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_DISABLED_BUTTON_COLOR)
        self.rightButtonNode.rippleColor = self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_PRIMARY_TEXT_COLOR).withAlphaComponent(0.05)
        self.backButtonArrow.image = NavigationBar.backArrowImage(color: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR))
        if let title = self.title {
            self.titleNode.attributedText = NSAttributedString(string: title, font: NavigationBar.titleFont, textColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_PRIMARY_TEXT_COLOR))
            self.titleNode.accessibilityLabel = title
        }
        self.stripeNode.backgroundColor = self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_SEPARATOR_COLOR)

        self.backgroundNode = NavigationBackgroundNode(color: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BLURRED_BACKGROUND_COLOR))
    ...
    public func updatePresentationData(_ presentationData: NavigationBarPresentationData, transition: ContainedViewLayoutTransition = .immediate) {
        ...
        if let theme = presentationData.theme, theme !== self.presentationData.theme {
            
            self.presentationData = presentationData
            self.backgroundNode.updateColor(color: theme.backgroundColor, transition: transition)
            
            self.backButtonNode.color = theme.buttonColor
            self.backButtonNode.disabledColor = theme.disabledButtonColor
            self.leftButtonNode.color = theme.buttonColor
            self.leftButtonNode.disabledColor = theme.disabledButtonColor
            self.rightButtonNode.color = theme.buttonColor
            self.rightButtonNode.disabledColor = theme.disabledButtonColor
            self.rightButtonNode.rippleColor = theme.primaryTextColor.withAlphaComponent(0.05)
            self.backButtonArrow.image = NavigationBar.backArrowImage(color: theme.buttonColor)
            if let title = self.title {
                self.titleNode.attributedText = NSAttributedString(string: title, font: NavigationBar.titleFont, textColor: theme.primaryTextColor)
                self.titleNode.accessibilityLabel = title
            }
            self.stripeNode.backgroundColor = theme.separatorColor
            
            self.badgeNode.updateTheme(fillColor: theme.buttonColor, strokeColor: theme.buttonColor, textColor: theme.badgeTextColor)
    ...
    public func makeTransitionBadgeNode() -> ASDisplayNode? {
        if self.badgeNode.supernode != nil && !self.badgeNode.isHidden {
            let node = NavigationBarBadgeNode(fillColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR), strokeColor: self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BUTTON_COLOR), textColor:  self.resourceProvider.getColor(key: KEY_NAVIGATION_BAR_BADGE_TEXT_COLOR))
