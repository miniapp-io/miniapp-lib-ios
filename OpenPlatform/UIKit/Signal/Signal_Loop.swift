...
internal enum SignalFeedbackLoopState<T> {
    case initial
    case loop(T)
}

internal func feedbackLoop<R1, R, E>(once: @escaping (SignalFeedbackLoopState<R1>) -> Signal<R1, E>?, reduce: @escaping (R1, R1) -> R1) -> Signal<R, E> {
