...
internal func attachmentDefaultTopInset(layout: ContainerViewLayout?) -> CGFloat {
...
internal class AttachmentContainer: ASDisplayNode, ASGestureRecognizerDelegate {
    ...
    var allowVerticalSwipe: (() -> Bool)?
    var allowHorizontalSwipe: (() -> Bool)?
    
    var shouldCancelPanGesture: (() -> Bool)?
    var isModalStyle: (() -> Bool)?
    var allowExpand: (() -> Bool)?
    var isFullScreenMod: (() -> Bool)?
    ...
    private var isVerticalSwipe: Bool? // 用于记录滑动方向
    private var poinBeginTouch : CGPoint?
    
    private func checkSwipe(_ isVerticalSwipe: Bool) -> Bool {
        if isVerticalSwipe {
            if !checkVerticalSwipe() {
                return false
            }
        } else {
            if !checkHorizontalSwipe() {
                return false
            }
        }
    ...
    private func checkHorizontalSwipe() -> Bool {
        if let allowHorizontalSwipe = self.allowHorizontalSwipe, allowHorizontalSwipe() {
            return true
        }
        return false
    }
    
    private func checkVerticalSwipe() -> Bool {
        if let allowVerticalSwipe = self.allowVerticalSwipe, allowVerticalSwipe() {
            return true
        }
        return false
    }
    ...
    @objc func panGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            // 重置滑动方向
            self.isVerticalSwipe = nil
            
            let point = recognizer.location(in: self.view)
            let currentHitView = self.hitTest(point, with: nil)
            self.poinBeginTouch = point
        ...
        case .changed:
            ...
            // 确定滑动方向（垂直或水平）
            if self.isVerticalSwipe == nil {
                let isHorizontalSwipe = abs(translation.y) < abs(translation.x)
                if isHorizontalSwipe && (self.poinBeginTouch?.x ?? 100) < 100 {
                    self.isVerticalSwipe = false
                }
            }
            
            // 如果是右滑
            let isVerticalSwipe = self.isVerticalSwipe ?? true
            if !isVerticalSwipe {
                if !self.checkHorizontalSwipe() {
                    return
                }
                translationY = translation.x * 2
            } else {
                if !self.checkVerticalSwipe() {
                    return
                }
            }
            ...
            var currentOffset = topInset + translationY
            ...
            if isVerticalSwipe {
                ...
                if case let .known(value) = visibleContentOffset, value <= epsilon {
                    ...
                    translationY = panOffset
                    currentOffset = topInset + translationY
                    if self.isExpanded {
            ...
            self.panGestureArguments = (topInset, translationY, scrollView, listNode)
            ...
            if (!self.isExpanded || !isVerticalSwipe), translationY > 40.0, let shouldCancelPanGesture = self.shouldCancelPanGesture, shouldCancelPanGesture() {
            ...
            if self.isExpanded && false == self.isFullScreenMod?() {
                bounds.origin.y = -max(0.0, translationY - edgeTopInset)
            } else {
                bounds.origin.y = -translationY
            }
        ...
        case .ended:
            ...
            if (bounds.minY < -60 || (bounds.minY < 0.0 && velocity.y > 300.0) || (self.isExpanded && bounds.minY.isZero && velocity.y > 1800.0) || (self.isExpanded && !isVerticalSwipe && bounds.minY.isZero && translationY > 60.0) ) && !ignoreDismiss {
                self.interactivelyDismissed?()
                dismissing = true
            } else if self.isExpanded {
                if !(self.isFullScreenMod?() ?? false) && isVerticalSwipe && (velocity.y > 300.0 || offset > topInset / 2.0) {
    ...
    func requestFullScreen(_ show: Bool) {
        self.isExpanded = true
    }
    ...
    func update(layout: ContainerViewLayout, controllers: [AttachmentContainable], coveredByModalTransition: CGFloat, transition: ContainedViewLayoutTransition, completion: @escaping () -> Void = {}) {
        ...
        var isFullScreenMod = self.isFullScreenMod?() ?? false
        
        let topInset: CGFloat
        if !isFullScreenMod, let (panInitialTopInset, panOffset, _, _) = self.panGestureArguments {
        ...
        if case .compact = layout.metrics.widthClass {
            ...
            if isLandscape || isFullScreenMod {
